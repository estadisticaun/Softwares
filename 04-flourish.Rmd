...
### ¿Cómo compartir el trabajo?{#compartirtrabajoflourish}
...
1.	Luego de seleccionada la plantilla de gráfico de líneas debe hacer clic sobre subir datos, navegar en las carpetas, seleccionar el archivo “evoluciongeneral.csv” y hacer clic en abrir para cargar los datos, luego de esto se presentará un aviso en su pantalla en el que se informa que los datos serán visibles públicamente debido a que se está usando la versión gratuita, debe hacer clic en el botón importar públicamente.
...
4.	La primera pestaña para explorar y dar formato al gráfico será Colors (colores), allí debe desplegar el campo paleta y seleccionar editar paleta de colores, elimine los colores que posee esta paleta dejando solo uno el cual debe reemplazar con “#358DC8”.
...
8.	Nuevamente sobre la pestaña eje X se encuentra una sección dedicada a las marcas de graduación y líneas del eje, cambie el color de línea por “#BDC3C7”, active las líneas de cuadrícula y su botón de estilo, allí debe asignar el mismo color de línea en un estilo punteado.
...
```{r asignaciondecamposflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Asignar columnas a los estantes correspondientes', out.width='85%'}
...
8.	Debe repetir los pasos 14, 15 y 16 del gráfico de líneas, para el paso 15 el título de la visualización debe ser Evolución del número de estudiantes graduados por modalidad de formación y para el paso 16 será Modalidad de formación.
...
1.	Desde la galería de plantillas debe seleccionar gráfico circular, cargar la fuente de datos llamada “modalidadformacioncircular.csv”, para el estante Labels/Time debe asignar la columna B ya que esta contiene la modalidad de formación, la columna E debe ir en el estante Values debido a que se quiere mostrar los porcentajes, la columna A será usada como filtro de fila, finalmente al estante de información para ventanas emergentes personalizadas se deben asignar las columnas B y C.
...
7.	Con ayuda de la sección encabezamiento seleccione alineación central y escriba como título Distribución de graduados por modalidad de formación, periodo, este título debe estar en negrita y su tamaño debe ser la A mayúscula más grande.
...
```{r graficocircularconfiltroflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Distribución de estudiantes graduados por modalidad de formación', out.width='85%'}
...
6.	Es momento de editar los ejes para esto haga clic sobre eje X y asigne como título Número de graduados en color negro, negrita y tamaño 1.3, las etiquetas también deben estar en color negro y tamaño 1; también debe activar las líneas de cuadrícula.
...
5.	Otra pestaña para editar y personalizar el gráfico es caja de búsqueda, con esta se da formato a la opción de búsqueda; el texto del marcador será “Buscar…” y debe activar las opciones de mostrar sugerencias desplegables y expandir el cuadro de búsqueda.
...
2.	La siguiente pestaña para editar es popups y paneles, en la cual debe seleccionar contenido emergente personalizado, el encabezado será el nombre del municipio y departamento y el contenido será la cantidad de graduados y para la ventana emergente relacionada a los puntos no debe editar nada.
...
```{r paso8mapa3dflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Edición de la línea de tiempo', out.width='85%'}
...
```{r paso5jerarquiaflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Cambio de paleta de colores', out.width='85%'}
...
```{r paso8jerarquiaflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Formato de número', out.width='85%'}
...
#### Carrera de gráfico de barras {#carrerabarrasflourish}
...
3.	Luego de añadir una imagen notara que en la fila correspondiente y columna imágenes se genera un enlace que corresponde a la imagen, puede copiar y pegarlo hasta completar todas las filas que corresponden a esa área. Debe repetir el paso 2 y lo mencionado anteriormente hasta completar todas las filas de la columna imágenes.
...
5.	Regresando a la pestaña avance se encuentran algunos problemas de formato, tales como el título y colores de la leyenda y el nombre de las áreas se corta, están incompletos.
...
6.	Para iniciar a modificar y personalizar el gráfico, debe hacer clic sobre la pestaña colores de barra y editar la paleta de colores, “#6D6666”, “#FBB03B”, “#5BC0DE”, “#C1272D”, y “#8CC63F” deben ser asignados a los niveles doctorado, especialidades médicas, especialización, maestría y pregrado respectivamente.
...
#### Carrera de gráfico de líneas {#carreradelineasflourish}
...
1.	Cargue el archivo de datos llamado “car_lineas.csv” y asigne las columnas Región y Departamento a los estantes filtro y nombre de línea respectivamente; deberá insertar una nueva columna a la izquierda de la columna C, la cual se llamará imágenes y debe adjuntar la imagen de la bandera correspondiente a cada departamento y luego asignar esta columna al estante imagen, finalmente las columnas D a la R deben ubicarse en el estante puntajes de columnas. 

```{r paso1carlineasflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Creación de columna imágenes y asignación a estantes', out.width='85%'}
knitr::include_graphics("Imágenes/flourish158.jpg")
```

2.	Regresando a la pestaña avance o previa se observan algunos inconvenientes, tales como el tamaño de los textos, el idioma de los botones y las márgenes; una solución rápida para ampliar un poco el gráfico es cambiar el nombre Archipiélago de san andrés, providencia y santa catalina por San andrés, esto lo puede hacer desde el panel datos.

```{r paso2carlineasflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Vista previa de la visualización', out.width='85%'}
knitr::include_graphics("Imágenes/flourish159.jpg")
```

3.	La primera pestaña disponible para dar formato al gráfico es vista, con la cual se controla la cantidad de puntos de la ventana de tiempo que se muestran, la primera opción es mostrar todo, es decir, se muestran al tiempo todos los puntos de la línea de tiempo y la opción Zoomed permite mostrar la cantidad de puntos deseada haciendo que el eje de tiempo sea dinámico; en este caso los puntos en el tiempo no son demasiados por lo que se debe dejar en mostrar todo.

4.	Con ayuda de la pestaña tamaño del gráfico debe seleccionar como modo de altura llenar espacio, también se harán modificaciones en las márgenes, la margen derecha debe establecerse en 1, la izquierda en 2 y las demás opciones en 0.

```{r paso4carlineasflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Edición de tamaño del gráfico', out.width='85%'}
knitr::include_graphics("Imágenes/flourish160.jpg")
```

5.	Es momento de dar formato a los controles de selección y filtro, haga clic en la pestaña controles, inicialmente en la sección filtrar deberá desactivar la opción de incluir todo, seguido de esto se debe cambiar el nombre de las etiquetas por rangos, cantidad y repetir respectivamente, por último, en la sección estilos de grupos de botones deberá cambiar el color de seleccionado por “#808080”.

```{r paso5carlineasflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Formato a los controles', out.width='85%'}
knitr::include_graphics("Imágenes/flourish161.jpg")
```

6.	Con ayuda de la pestaña estilos de línea deberá aumentar el ancho de línea a 0.2, haga clic en la pestaña estilos de círculos y aumente los radios de inicio y final a 0.4 y 1 respectivamente, adicional a esto para el color de trazo debe seleccionar fondo.

```{r paso6carlineasflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Estilos de línea y círculo', out.width='85%'}
knitr::include_graphics("Imágenes/flourish162.jpg")
```

7.	La pestaña estilos de etiqueta permite controlar el color y tamaño de las etiquetas, en este caso debe aumentar tanto el tamaño de fuente de clasificación como el de la etiqueta a 1 y el color de la etiqueta debe ser reparado (fixed).

```{r paso7carlineasflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Estilo de etiquetas', out.width='85%'}
knitr::include_graphics("Imágenes/flourish163.jpg")
```

8.	Para el eje Y debe aumentar el tamaño de etiqueta a 1 y activar la opción de eje dinámico, esto para que dependiendo del máximo y mínimo de cada región geográfica el eje Y se adecue.

```{r paso8carlineasflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Formato al eje Y', out.width='85%'}
knitr::include_graphics("Imágenes/flourish164.jpg")
```

9.	Para el eje X el tamaño de la etiqueta también será de 1 y el ángulo debe establecerse en 30°. Finalmente haciendo uso de la pestaña diseño seleccione como color de fondo “#ECF0F1” y active los bordes en color negro; escriba como título Evolución histórica del total de estudiantes graduados por departamentos en alineación central.
...
<div class="flourish-embed flourish-chart" data-src="visualisation/5913710"><script src="https://public.flourish.studio/resources/embed.js"></script></div>
...
```{r paso3pendienteflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Desactivar leyenda y formato a las etiquetas', out.width='85%'}
...
```{r asignarcolumnasradarflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Asignación de columnas a estantes disponibles', out.width='85%'}
knitr::include_graphics("Imágenes/flourish133.jpg")
```
...
1.	Con la pestaña eje radial es posible dar formato al eje del gráfico, inicialmente debe cambiar el color de las líneas por “#646464” y aumentar el ancho a 0.15; el color de las etiquetas también debe ser “#646464” y su tamaño debe ser reducido a 1; las líneas y anillos deben ser de color “#808080” al igual que la etiqueta Y.

```{r paso1radarflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Edición del eje radial', out.width='85%'}
knitr::include_graphics("Imágenes/flourish134.jpg")
```

2.	La pestaña controles permite cambiar forma, colores y texto de los filtros incluidos en la visualización, para iniciar cambie el nombre de la etiqueta combinada y separada por Combinado y Separado respectivamente y para el filtro de facultades la opción All debe ser cambiada por Todos. Finalmente debe dirigirse a la sección de estilos de botones y cambiar el color de seleccionado por “#808080”.

```{r paso2radarflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Formato a los controles', out.width='85%'}
knitr::include_graphics("Imágenes/flourish135.jpg")
```

3.	Haciendo uso de la pestaña diseño asigne “#ECF0F1” como color de fondo y también active las fronteras, estas deben ser de color negro. Por último, se debe incluir un título y subtítulo en la visualización, los cuales serán Resultados Saber Pro 2019, Bogotá y Competencias genéricas respectivamente.
...
```{r radarbogotaflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Gráfico de radar sede Bogotá facultad de ciencias económicas', out.width='85%'}
knitr::include_graphics("Imágenes/flourish136.jpg")
```
...
#### Diagramas de dispersión {#dispersionflourish}
...
1.	Seleccione la plantilla de diagrama de dispersión básico y cargue el archivo de datos mencionado anteriormente, asigne las columnas U y R a los estantes valores de X y valores de Y respectivamente, con esto obtendrá un gráfico de dispersión básico.

```{r dispersionbasicoflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Gráfico de dispersión básico para beneficos vs ventas', out.width='85%'}
knitr::include_graphics("Imágenes/flourish137.jpg")
```
...
```{r dispersionbasicotendenciaflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Gráfico de dispersión básico para beneficos vs ventas con línea de tendencia', out.width='85%'}
knitr::include_graphics("Imágenes/flourish142.jpg")
```

2.	Si se añade una columna categórica al estante cuadrícula de gráficos, se obtiene una visualización con tantos gráficos como categorías posea la columna, por ejemplo, añada la columna Category al estante mencionado y obtendrá la siguiente visualización.

```{r minidispersioncotendenciaflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Minigráficos por categoría con líneas de tendencia', out.width='85%'}
knitr::include_graphics("Imágenes/flourish138.jpg")
```
...
```{r minidispersioncolortendenciaflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Columnas para controlar el color y tamaño de los puntos', out.width='85%'}
knitr::include_graphics("Imágenes/flourish139.jpg")
```

3.	Para realizar el gráfico de burbujas se debe retirar la columna ubicada en el estante cuadrícula de gráfico y añadir la columna Product Name al estante nombre, active la leyenda para que en la parte superior aparezca el texto que identifica a cada color y desactive la línea de tendencia.

```{r graficoburbujaflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Gráfico de burbuja', out.width='85%'}
knitr::include_graphics("Imágenes/flourish140.jpg")
```
...
```{r graficoburbujaconfiltroflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Gráfico de burbuja con filtro de región', out.width='85%'}
knitr::include_graphics("Imágenes/flourish141.jpg")
```
5.	Para realizar el diagrama de Hans Roseling se debe agregar una columna de tiempo al estante tiempo, en este caso dicha columna será Order Date que corresponde a la fecha en la cual el cliente realiza el pedido, únicamente se seleccionaron los años de dicha fecha ya que se tienen registros diarios y la visualización no era informativa, ya que solo presentaría pocos puntos.

```{r hansroselingflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Gráfico de Hans Roseling', out.width='85%'}
knitr::include_graphics("Imágenes/flourish143.jpg")
```
...
```{r formatopuntosflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Formato a los puntos', out.width='85%'}
knitr::include_graphics("Imágenes/flourish144.jpg")
```
...
```{r cambiopaletacoloresflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Cambio de paleta de colores', out.width='85%'}
knitr::include_graphics("Imágenes/flourish145.jpg")
```
...
```{r formatoejexdispersionflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Formato al eje X', out.width='85%'}
knitr::include_graphics("Imágenes/flourish146.jpg")
```
...
```{r formatoleyendadispersionflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Formato de leyenda', out.width='85%'}
knitr::include_graphics("Imágenes/flourish147.jpg")
```

-	Ahora con ayuda de la pestaña formato de número debe seleccionar la opción que tenga como separador de miles el punto.

```{r formatonumerodispersionflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Formato de número', out.width='85%'}
knitr::include_graphics("Imágenes/flourish148.jpg")
```
...
<div class="flourish-embed flourish-scatter" data-src="visualisation/5847335"><script src="https://public.flourish.studio/resources/embed.js"></script></div>
...
1.	Seleccione la plantilla gráfica de puntos y cargue el archivo de datos llamado “graficopuntos.csv”. Asigne las columnas SEDE_NOMBRE_MAT y n a los estantes valores de X y valores de Y respectivamente, adicional a esto asigne todas las columnas de la base de datos a información para ventanas emergentes personalizadas.

```{r paso1puntosflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Asignación de columnas a estantes', out.width='85%'}
knitr::include_graphics("Imágenes/flourish149.jpg")
```

2.	Al regresar a la pestaña avance la visualización muestra la cantidad de estudiantes graduados por sede de matrícula, pero se identifican algunos problemas con los títulos de los ejes y la descripción emergente.

```{r paso2puntosflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Vista previa de la visualización', out.width='85%'}
knitr::include_graphics("Imágenes/flourish150.jpg")

```

3.	Inicialmente haciendo uso de la pestaña estilo de puntos debe disminuir la opacidad del punto a 0.2, con el fin de generar más transparencia en cada punto y poder observar lo puntos que se sobreponen.

```{r paso3puntosflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Formato al estilo de puntos', out.width='85%'}
knitr::include_graphics("Imágenes/flourish151.jpg")
```

4.	Usando la pestaña configuraciones de gráfico active el color de fondo y deje el color predeterminado, el cual es “#F5F5F5”.

5.	Para el eje X debe seleccionar título personalizado y en el espacio de texto escribir “Número de gradauados”, active el estilo y seleccione negrita en color negro; para las etiquetas debe activar la opción de estilo, aumentar el tamaño a 1, color negro y ángulo de cero; adicional a esto debe activar las líneas de cuadrículas, activar su estilo, cambiar el color por “#BBBBB9” y como estilo seleccione punteada.

```{r paso5puntosflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Formato al eje X', out.width='85%'}
knitr::include_graphics("Imágenes/flourish152.jpg")
```

6.	Repita lo mencionado en el paso anterior para dar formato al eje Y, debe tener en cuenta que el título de este eje es Sede de matrícula.

7.	Es momento de cambiar el contenido de las ventanas emergentes, para esto debe seleccionar contenido personalizado en la pestaña de ventanas emergentes las columnas a incluir en esta descripción emergente son periodo, sede de matrícula y cantidad de estudiantes graduados; recuerde que se debe usar los códigos HTML para negrita y enter.

```{r paso7puntosflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Contenido personalizado a ventanas emergentes', out.width='85%'}
knitr::include_graphics("Imágenes/flourish153.jpg")
```

8.	La pestaña Box, violin and beeswarm plots permite añadir estas opciones gráficas a los puntos, por ejemplo, al seleccionar la opción automático de la sección beeswarm, se dispersan los puntos en cada una de las sedes.

```{r paso8puntosflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Beeswarm', out.width='85%'}
knitr::include_graphics("Imágenes/flourish154.jpg")
```

9.	Sobre esta misma pestaña se puede activar la opción de Boxplot y se obtiene un diagrama de cajas sobre cada una de las sedes.

```{r paso9puntosflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Boxplot', out.width='85%'}
knitr::include_graphics("Imágenes/flourish155.jpg")
```

10.	Finalmente es posible añadir el gráfico de violín, en este caso debe cambiar su color por “#808080” para lograr visualizarlos, también es de destacar que no hay información sobre la creación de estas sombras o violines.

```{r paso10puntosflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Violín', out.width='85%'}
knitr::include_graphics("Imágenes/flourish156.jpg")
```

11.	Finalmente, con la ayuda de la pestaña diseño active las fronteras y seleccione para todo el color negro; con la pestaña encabezamiento escriba como título Cantidad de graduados por sede de matrícula.
...
```{r finalpuntosflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Distribución de estudiantes graduados por sede de matrícula', out.width='85%'}
knitr::include_graphics("Imágenes/flourish157.jpg")
```
...
```{r pantallaincialhistoriaflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Página inicial creación de historias', out.width='85%'}
knitr::include_graphics("Imágenes/flourish165.jpg")
```

1.	La primera diapositiva tendrá la evolución histórica del total de estudiantes graduados, haga clic sobre elegir visualización y haga clic sobre el gráfico llamado Evolución graduados.

```{r paso1historiaflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Añadir una visualización', out.width='85%'}
knitr::include_graphics("Imágenes/flourish166.jpg")
```

2.	Al añadir una visualización obtendrá lo siguiente, lo cual corresponde a un estilo de diapositiva con la visualización seleccionada.

```{r paso2historiaflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Primera visualización', out.width='85%'}
knitr::include_graphics("Imágenes/flourish167.jpg")
```

3.	El icono de lápiz permite añadir anotaciones a la visualización, para demarcar puntos o acontecimientos importantes, por ejemplo, se puede añadir una anotación o mensaje en el punto más bajo de la línea indicando el acontecimiento ocurrido en ese periodo que produjo esa cantidad de estudiantes graduados. Para añadir una anotación haga clic sobre el lápiz y luego clic sobre el lugar de la visualización donde quiere ubicar el mensaje.

```{r paso3historiaflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Añadir anotaciones', out.width='85%'}
knitr::include_graphics("Imágenes/flourish168.jpg")
```

4.	Luego de esto aparecerán opciones para la anotación en la sección en blanco mostrada en la imagen anterior, entre las opciones hay un cuadro de texto, las opciones para mostrar la anotación en las cuales se incluye siempre, modo escritorio y dos opciones para celular, diferentes tipos de marcadores y conectores.

```{r paso4historiaflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Alternativas para las anotaciones', out.width='85%'}
knitr::include_graphics("Imágenes/flourish169.jpg")
```

5.	De esta manera es posible añadir todas las visualizaciones que se deseen a la historia, en este caso se creara una historia de 15 puntos o diapositivas, entre las cuales se seleccionaron gráficos de líneas, circulares, barras, mapas, carreras de barras y líneas, entre otros.
...
```{r paso6historiaflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Auto reproducción y control de tiempo', out.width='85%'}
knitr::include_graphics("Imágenes/flourish170.jpg")
```

7.	En cada diapositiva o punto de la historia se ofrece la posibilidad de añadir encabezados o mensajes en la parte superior, al lado de los controles de reproducción, primero se debe escribir el texto y luego seleccionándolo todo es posible cambiar el estilo a negrita, cursiva o subrayado.

```{r paso7historiaflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Añadir descripciones o encabezados', out.width='85%'}
knitr::include_graphics("Imágenes/flourish171.jpg")
```
...
```{r compartirhistoriaflourish-fig, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Compartir la historia creada', out.width='85%'}
knitr::include_graphics("Imágenes/flourish172.jpg")
```

Esta es la historia final, insertada directamente con el código HTML generado.

<div class="flourish-embed" data-src="story/836426"><script src="https://public.flourish.studio/resources/embed.js"></script></div>
